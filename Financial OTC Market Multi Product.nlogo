...
globals [ a returns p0s p1s price-historys histogram-num-bars obs-length ai-trained-str]

breed [ valueinvestors valueinvestor ]
breed [ smartinvestors smartinvestor ]
breed [ dealers dealer ]

turtles-own [ expectations inventorys ]
links-own [ weight ] ; weight will represent the size of trade between two parties for visual illustration
dealers-own [ bids offers last-trades last-traded-product ] ; bid and offer prices
valueinvestors-own [ uncertainty ]
smartinvestors-own [ confident? trade-holding-times state-memory positions product_idxs transaction-prices transaction-products ctransaction-price recent-price-historys actions_index ]
...
to setup ; global procedure
  ...
  ; initialise containers to hold each product's information (as nested lists)
  let idx 0
  set p0s []
  set returns [] ; list for recording the returns
  set price-historys []
  while [ idx < n-products ] [
    set p0s price-level idx ; cache the price level
    set returns lput [] returns
    set price-historys lput [] price-historys
    set idx idx + 1
  ]
...
to go ; global procedure
  ...
  ask one-of turtles [
    ...
    if ( breed = dealers ) and ( abs ( inventorys ) > dealer-position-limit ) and enable-broker-market? [ ;
      dealer-act
    ]
  ]
...
to update-pricehistory ; global procedure
  ; append price history for each product by iterating through loop
  let idx 0
  while [ idx < n-products ] [
    ; update the global price historys
    set price-historys ( replace-item idx price-historys lput mean [ item idx expectations ] of dealers item idx price-historys )
    ; update the recent price historys of each smart investor
    ask smartinvestors [
      set recent-price-historys ( replace-item idx recent-price-historys lput ( mean [ item idx expectations ] of link-neighbors with [ breed = dealers ] - 100 ) item idx recent-price-historys ) ; -100 to make neural network inputs closer to 0
      if length item idx recent-price-historys > obs-length [
        set recent-price-historys replace-item idx recent-price-historys remove-item 0 item idx recent-price-historys ; keep the recent price history a fixed length
...
to update-graphics ; global procedure
    ...
    ask dealers [
      if max inventorys > dealer-position-limit [
        set color green
      ]
      if min inventorys < -1 * dealer-position-limit [
        set color red
      ]
      if abs ( inventorys ) < dealer-position-limit [
        set color grey + 3
      ]
    ]
...
to record-returns ; global procedure
    ...
    let idx 0
    while [ idx < n-products ] [
      set p1s ( replace-item idx p1s price-level idx )
        set returns ( replace-item idx returns lput ( item idx p1s - item idx p0s ) returns )
        set p0s ( replace-item idx p0s item idx p1s )
      ]
  ]
end
...
to step ; global procedure
  ...
  let bestoffers []
  while [ idx < n-products ] [
   set bestbids lput max [ item idx bids ] of dealers bestbids
   set bestoffers lput min [ item idx offers ] of dealers bestoffers
  ]
end
...
to refresh-bidoffer ; dealer procedure
  let idx 0
  while [ idx < n-products ] [
    let axe-adj min list bid-offer a * sensitivity-function ( -1 * inventorys  )
    set expectations ( replace-item idx expectations ( ( item idx last-trades ) + axe-adj ) )

    set offers ( replace-item idx offers ( item idx expectations + bid-offer / 2 ) )
    set bids ( replace-item idx bids ( item idx expectations - bid-offer / 2 ) )
  ]
end


to dealer-act ; dealer procedure
  ; find the "worst" product for the dealer according to which product breaks the position limit the most
  let abs-inventorys map [ i -> abs i ] inventorys
  let idx position max abs-inventorys abs-inventorys; index of the "worst product"
  let inventory item idx inventorys
  ; set traded_size to bring dealer back within limits (so long as the trade is smaller than the global trade size limit)
...
to smartinvestor-act ; smart-investor procedure
  ...
  ifelse length trade-holding-times > 0 and min trade-holding-times <= ticks [ ; if a trade has ended, close out deterministically and remember + learn whether the action from the state went well
    ...
    let action_index item index actions_index ; the action associated with the closing trade
    let ptransaction-price ( item index transaction-prices ) ; the price associated with the closing trade
    let _position ( item index positions ) ; the size associated with the closing trade
    let product_index ( item index product_idxs ) ; the product being closed out
    if _position > 0 [ ; if position > 0, the trade we're closing out was a "buy". Need to "sell" to close
      let bestdealer [ who ] of min-one-of link-neighbors with [ breed = dealers ] [ item product_index inventorys ]
      transact "sell" _position bestdealer product_index
    ]
    if _position < 0 [ ; if position < 0, the trade we're closing out was a "sell". Need to "buy" to close
      let bestdealer [ who ] of max-one-of link-neighbors with [ breed = dealers ] [ item product_index inventorys ]
      transact "buy" -1 * _position bestdealer product_index
    ]
    ...
    py:set "historys" recent-price-historys
    let state py:runresult "[i for sublist in historys for i in sublist]"
    py:set "state" state
...
to-report price-level [idx]
  report mean [ item idx expectations ] of dealers
end
