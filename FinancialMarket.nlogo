...
smartinvestors-own [ last-state ctransaction-price ptransaction-price prior-price-history confident? trade-holding-time starttime recent-price-history]
...
to setup
  ...
  set a 0.2 * bid-offer / mm-PositionLimit ; constant determining how much bid-offer increases as size increases. Model parameter
  ...
  create-ordered-smartinvestors nSmartInvestors [
    ...
    py:set "action_size" 2
    py:set "eps_decay" 0.9995
    py:run "agents[id] = q.SmartTrader(lr, state_size, action_size, eps_decay=eps_decay, batch_size=64)"
    set model_built true
    set trade-holding-time 0

    set starttime random smart-holdingtime ; initialise randomly so smart-investors trades occur evenly distributed

    set ctransaction-price 0 ; initialise
    set ptransaction-price 0 ; initialise
    set prior-price-history []
    set confident? false
    set turn-taken? false

    if AI-investor-model = "Pretrained" [
      py:set "counter" counter
      py:run "modelname = 'RLtrader{}.pt'.format(counter)"
...
to go
  ...
  ask smartinvestors [
    set trade-holding-time trade-holding-time + 1
  ]
...
to smartinvestor-act
  ...
  let positioned ( abs ( long - short ) != 0 )
  py:set "id" who
  py:set "state" prior-price-history ; state is price history
  let paction 0
  ...
  if positioned and trade-holding-time >= smart-holdingtime [ ; if positioned, close out deterministically and remember + learn whether the prior action went well. Else use the AI to choose an action
    let _position ( long - short )
    set ptransaction-price ctransaction-price
    if _position > 0 [
      smartinvestor-sell
      set paction 1 ; if theyr'e selling now, the action was previously to buy
    ]
    if _position < 0 [
      smartinvestor-buy
      set paction 0
    ...
    set bidofferpaid bidofferpaid + bid-offer * 0.5

    py:set "reward" _position * ( ctransaction-price - ptransaction-price )  ; exclude the commission from the reward
    py:set "action" paction
    py:run "agents[id].remember(state, action, reward)"
    py:run "agents[id].learn()"
  ]

  if not positioned and ticks > starttime [

    let action py:runresult "agents[id].act(state)"
    if action != "DO NOTHING" [
      set prior-price-history recent-price-history ; save the state associated with the action
      if action = 0 [
        set totalsmartbuy totalsmartbuy - 1
        smartinvestor-sell
      ]
      if action = 1 [
        set totalsmartbuy totalsmartbuy + 1
...
to refresh-bidoffer ; dealer procedure
  ...
  let axe-adj 0
  if short - long > 0 [
    set axe-adj min list bid-offer a * sensitivity-function ( short - long  )
  ]

  if long - short > 0 [
    set axe-adj -1 * min list bid-offer a * sensitivity-function ( long - short )
  ]

  set expectation ( expectation * ( 100 - price-sensitivity ) + last-trade * price-sensitivity ) / 100 + min list bid-offer axe-adj

  let stochasm 0 ;( ( random-float 1 ) - 0.5) * bid-offer / 5

  set offer ( expectation + bid-offer / 2 ) + stochasm
  set bid ( expectation - bid-offer / 2 ) + stochasm
...
to smartinvestor-sell
  ...
  let tradesize smartsize

  if long - short > 0 [
    set tradesize ( long - short )
  ]
