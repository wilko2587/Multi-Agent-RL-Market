...
filename = 'fragmentation-mean.csv'
data = pd.read_csv(filename).iloc[:, 1:].dropna(axis=1)
data.index = ['start px', 'end px', 'mean expectation', 'time averaged price', 'mean abs deviation', 'prob-of-link', 'kurtosis', 'skew']

data.loc['Deviation from mean expectation'] = (data.loc['mean expectation'] - data.loc['time averaged price']).abs()
data.loc['Deviation from mean expectation'] = data.loc['mean abs deviation']
...
sns.regplot(data=data, x='prob-of-link', y='Deviation from mean expectation', ax=axs, order=2, marker='o', line_kws={'linestyle':'solid'}, color='black')
